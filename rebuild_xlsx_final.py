...
Final version: Correct VLOOKUP column indices.
...
Données A65:J68 columns:
  A=1: Enterprise name
  B=2: Icon
  C=3: Sector
  D=4: Special effect
  E=5: Initial actif total
  F=6: Capitaux propres (AVANT)
  G=7: Emprunts (AVANT)
  H=8: Dettes (AVANT)
  I=9: Découvert (AVANT)
  J=10: Résultat initial (AVANT)
"""
...
def build_turn_block(ws, turn_num):
    ...
    # Actif rows - Données columns: 2=Immo, 3=Stocks, 4=Créances, 5=Tréso
    actifs = [
        (40, 'Immobilisations', 2),
        (41, 'Stocks', 3),
        (42, 'Créances clients', 4),
        (43, 'Trésorerie', 5),
        (44, 'TOTAL ACTIF', None),
    ]

    for offset, label, col_idx in actifs:
        r = bs + offset
        ws[f'A{r}'] = label
        ws[f'A{r}'].alignment = Alignment(horizontal='left')
        ...
        if turn_num == 1:
            if col_idx:
                ws[f'C{r}'] = f'=IFERROR(VLOOKUP($D$3,Données!$A$65:$J$68,{col_idx},0),0)'
            else:
    ...
    # Passif rows - Données columns: 6=Capitaux, 7=Emprunts, 8=Dettes, 9=Découvert, 10=Résultat
    passifs = [
        (47, 'Capitaux propres', 6),
        (48, 'Emprunts', 7),
        (49, 'Dettes fournisseurs', 8),
        (50, 'Découvert', 9),
        (51, 'Résultat', 10),
        (52, 'TOTAL PASSIF', None),
    ]

    for offset, label, col_idx in passifs:
        r = bs + offset
        ws[f'A{r}'] = label
        ws[f'A{r}'].alignment = Alignment(horizontal='left')
