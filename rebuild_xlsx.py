...
Structure:
- Header rows 1-11: config zone
- Turn blocks: 12 rows for config + per-turn data
  - Each block is BLOCK_SIZE=67 rows
...
# Offsets within each turn block
OFFSETS = {
    'turn_header': 0,
    'input_emprunt': 3,
    'input_achat': 4,
...
def read_donnees_sheet(xlsx_path):
    ...
    try:
        ...
        if 'Données' not in wb.sheetnames:
            ...
            return None
        sheet = wb['Données']
        # Read all cells to preserve content
        data = {}
...
def build_turn_block(ws, turn_num):
    ...
    bs = HEADER_ROWS + 1 + (turn_num - 1) * BLOCK_SIZE  # Block start row (Excel 1-indexed)

    # Turn header (offset 0)
    ...
    # Décisions section header (offset 1)
    r = bs + 1
    ws.merge_cells(f'A{r}:G{r}')
    cell = ws[f'A{r}']
    ...
    decisions = [
        ('input_emprunt', 'Emprunt demandé :', VALIDATION_LISTS['emprunt']),
        ('input_achat', 'Achat de stock :', None),
        ('input_mode', 'Mode de trésorerie :', VALIDATION_LISTS['mode']),
        ('input_recr', 'Recrutement :', VALIDATION_LISTS['recr']),
        ('input_carte', 'Carte action :', VALIDATION_LISTS['carte']),
        ('input_event', 'Événement :', VALIDATION_LISTS['event']),
    ]

    for key, label, dv_list in decisions:
        offset = OFFSETS[key]
        r = bs + offset
        ...
        cell = ws[f'D{r}']
        if key == 'input_emprunt':
            cell.value = 0
        elif key == 'input_achat':
            cell.value = 0
        elif key == 'input_mode':
            cell.value = 'Tréso'
        elif key == 'input_recr':
            cell.value = 'Aucun'
        elif key == 'input_carte':
            cell.value = 'Aucune'
    ...
    # Effets automatiques section header (offset 10)
    r = bs + 10
    ws.merge_cells(f'A{r}:G{r}')
    cell = ws[f'A{r}']
    ...
    # Bilan section header (offset 39)
    r = bs + 39
    ws.merge_cells(f'A{r}:G{r}')
    cell = ws[f'A{r}']
    cell.value = '📊 BILAN'
    ...
    # CR produits (offset 56-58)
    cr_produits = [
        ('cr_ca', 'CA ventes', '(+)', 'Produits', 'Unités × 2 000 €/u'),
        ('cr_prodexcept', 'Produits except.', '(+)', 'Produits', 'Subvention, remb. assurance'),
    ]

    for key, label, sign, cat, note in cr_produits:
        offset = OFFSETS[key]
        r = bs + offset
        ws[f'A{r}'] = label
        ws[f'B{r}'] = sign
        ws[f'D{r}'] = f'=D{bs+25}' if key == 'cr_ca' else f'=IFERROR(VLOOKUP($D${bs+7},Données!$A$2:$V$38,14,0),0)+IFERROR(VLOOKUP($D${bs+8},Données!$B$45:$L$57,8,0),0)'
        ws[f'F{r}'] = note
        set_number_format(ws[f'D{r}'])
        for col in ['A', 'B', 'D', 'F']:
            format_cell(ws[f'{col}{r}'], bg_color=COLORS['cr_produit'])

    # Total produits (offset 58)
    ...
    # CR charges (offset 59-63)
    cr_charges = [
        ('cr_cmv', 'CMV', '(−)', 'Charges', 'Stocks à valeur d\'achat'),
        ('cr_charpers', 'Ch. personnel', '(−)', 'Charges', 'Salaires commerciaux'),
        ('cr_charfixes', 'Ch. fixes & serv.', '(−)', 'Charges', 'Loyer -2000 + cartes'),
        ('cr_dotations', 'Dotations amort.', '(−)', 'Charges', 'Amortissements'),
        ('cr_charint', 'Ch. intérêt', '(−)', 'Charges', 'Agios + intérêts emprunt'),
    ]

    vlk_recr = f"IFERROR(VLOOKUP($D${bs+6},Données!$A$2:$V$38,9,0),0)"
    vlk_carte = f"IFERROR(VLOOKUP($D${bs+7},Données!$A$2:$V$38,9,0),0)"
    vlk_carte_fees = f"IFERROR(VLOOKUP($D${bs+7},Données!$A$2:$V$38,10,0),0)+IFERROR(VLOOKUP($D${bs+7},Données!$A$2:$V$38,11,0),0)"
    vlk_event = f"IFERROR(VLOOKUP($D${bs+8},Données!$B$45:$L$57,8,0),0)"
    vlk_carte_int = f"IFERROR(VLOOKUP($D${bs+7},Données!$A$2:$V$38,12,0),0)"

    for key, label, sign, cat, note in cr_charges:
        offset = OFFSETS[key]
        r = bs + offset
        ws[f'A{r}'] = label
        ws[f'B{r}'] = sign
        ...
        if key == 'cr_cmv':
            ...
            ws[f'D{r}'] = f'={vlk_recr}+{vlk_carte}'
        elif key == 'cr_charfixes':
            ws[f'D{r}'] = f'=-2000+{vlk_carte_fees}'
        elif key == 'cr_dotations':
            ws[f'D{r}'] = f'=D{bs+12}'
        elif key == 'cr_charint':
            ws[f'D{r}'] = f'=D{bs+14}+{vlk_carte_int}'
