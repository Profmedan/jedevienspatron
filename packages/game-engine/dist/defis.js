...
exports.piocherDefi = piocherDefi;
exports.appliquerChoixDefi = appliquerChoixDefi;
exports.resoudreConsequencesDifferees = resoudreConsequencesDifferees;
exports.formatContexte = formatContexte;
// ─── PIOCHE D'UN DÉFI ────────────────────────────────────────
/**
 ...
 */
function piocherDefi(etat, joueur, slot, catalogue, rng = Math.random) {
    const defisResolus = etat.defisResolus ?? [];
    const idsDejaResolusParJoueur = new Set(defisResolus
        .filter((r) => r.joueurId === joueur.id)
        .map((r) => r.defiId));
    const eligibles = catalogue.filter((defi) => {
        if (defi.slot !== slot)
            return false;
        if (etat.tourActuel < defi.tourMin)
            return false;
        if (defi.tourMax !== undefined && etat.tourActuel > defi.tourMax)
            return false;
        if (defi.entrepriseExclusive !== undefined &&
            joueur.entreprise.nom !== defi.entrepriseExclusive) {
            return false;
        }
        if (idsDejaResolusParJoueur.has(defi.id))
            return false;
 ...
 */
function appliquerChoixDefi(defi, choix, etat, joueur) {
    if (!defi.choix.some((c) => c.id === choix.id)) {
        throw new Error(`appliquerChoixDefi: choix "${choix.id}" absent du défi "${defi.id}".`);
    }
    // Construction de l'arc différé éventuel.
    let arcDiffereACreer = null;
 ...
 */
function resoudreConsequencesDifferees(etat) {
    const defisActifs = etat.defisActifs ?? [];
    const effetsAppliquer = [];
    const defisActifsRestants = [];
    for (const arc of defisActifs) {
        const dus = arc.effetsRestants.filter((e) => e.trimestreApplication <= etat.tourActuel);
        const futurs = arc.effetsRestants.filter((e) => e.trimestreApplication > etat.tourActuel);
...
function tresorerieNette(joueur) {
 ...
 */
function formatContexte(template, etat, joueur) {
