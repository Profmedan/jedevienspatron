...
exports.appliquerApprovisionnementVeloce = appliquerApprovisionnementVeloce;
exports.appliquerApprovisionnementSynergia = appliquerApprovisionnementSynergia;
...
exports.calculerCapaciteProductionBelvaux = calculerCapaciteProductionBelvaux;
exports.appliquerRealisationBelvaux = appliquerRealisationBelvaux;
exports.appliquerRealisationAzura = appliquerRealisationAzura;
exports.appliquerRealisationVeloce = appliquerRealisationVeloce;
exports.appliquerRealisationSynergia = appliquerRealisationSynergia;
exports.appliquerExtourneEnCours = appliquerExtourneEnCours;
 ...
 */
function pushByName(joueur, modifications, nomLigne, delta, explication) {
    ...
    const passif = !actif
        ...
        : undefined;
    if (!actif && !passif) {
        throw new Error(`[GameEngine] pushByName — ligne "${nomLigne}" introuvable pour le joueur ${joueur.pseudo}. ` +
    ...
    const categorie = actif ? actif.categorie : passif.categorie;
    modifications.push({
        joueurId: joueur.id,
        poste: categorie,
        ligneNom: nomLigne,
    ...
    if (immoAmortissables.length > 0) {
        ...
        immoAmortissables.forEach(({ a, idx }) => {
            ...
            const dotation = a.dureeAmortissement
                ...
                : types_1.AMORTISSEMENT_PAR_BIEN;
        ...
        push("dotationsAmortissements", totalDotation, `Dotation aux amortissements : +${totalDotation} € (${detailLibelles.join(", ")})`);
    }
...
function appliquerAchatMarchandises(etat, joueurIdx, quantite, modePaiement) {
    ...
    pushByName(joueur, modifications, nomLigneStock, montant, `Achat de ${quantite} unité(s) de ${libelleStock} (+${montant} €)`);
...
function appliquerApprovisionnementVeloce(etat, joueurIdx) {
    const joueur = etat.joueurs[joueurIdx];
    const modifications = [];
    ...
    const push = makePush(joueur, modifications);
    const montant = types_1.COUT_APPROCHE_VELOCE_PAR_TOUR;
    // DÉBIT : Services extérieurs (charge, CR)
    push("servicesExterieurs", montant, `Coût d'approche tournée : carburant, préparation véhicule, cotisations chauffeur (+${montant} €)`);
    // CRÉDIT : Dettes fournisseurs (passif, bilan)
    push("dettes", montant, `Dette fournisseur +${montant} € (règlement au trimestre suivant)`);
    return { succes: true, modifications };
}
...
function appliquerApprovisionnementSynergia(etat, joueurIdx) {
    const joueur = etat.joueurs[joueurIdx];
    const modifications = [];
    const modele = getModeleValeurEntreprise(joueur.entreprise);
    ...
    const push = makePush(joueur, modifications);
    const montant = types_1.COUT_STAFFING_SYNERGIA_PAR_TOUR;
    // DÉBIT : Services extérieurs (charge, CR)
    push("servicesExterieurs", montant, `Staffing mission : sous-traitance consultants, licences outil, kickoff (+${montant} €)`);
    // CRÉDIT : Dettes fournisseurs (passif, bilan)
    push("dettes", montant, `Dette fournisseur +${montant} € (règlement au trimestre suivant)`);
    return { succes: true, modifications };
}
...
function genererClientsDepuisFlux(flux) {
    ...
    const clients = [];
    for (const f of flux) {
        const idCarte = cartes_1.CLIENT_CARD_ID_BY_TYPE[f.typeClient];
...
function appliquerCarteClient(etat, joueurIdx, carteClient, saleGroupIndex) {
    ...
    const typeClient = cartes_1.TYPE_BY_CLIENT_CARD_ID[carteClient.id];
...
function appliquerRealisationMetier(etat, joueurIdx) {
    const joueur = etat.joueurs[joueurIdx];
    const modele = getModeleValeurEntreprise(joueur.entreprise);
    switch (modele.mode) {
        case "production":
...
function calculerCapaciteProductionBelvaux(joueur) {
    const nbCartes = joueur.cartesActives.filter((c) => CARTES_PRODUCTION_BELVAUX.has(c.id)).length;
    return types_1.PRODUCTION_BELVAUX_PAR_TOUR + nbCartes * BONUS_PRODUCTION_PAR_CARTE;
}
function appliquerRealisationBelvaux(etat, joueurIdx) {
    const joueur = etat.joueurs[joueurIdx];
    const modifications = [];
    const modele = getModeleValeurEntreprise(joueur.entreprise);
    ...
    const coutUnitaire = types_1.PRIX_UNITAIRE_MARCHANDISE; // 1 000 € par unité
    ...
    push("achats", montantTotal, `Variation de stock matières premières : consommation de ${quantiteEffective} unité${quantiteEffective > 1 ? "s" : ""} (+${montantTotal} € en charges)`);
    pushByName(joueur, modifications, "Stocks matières premières", -montantTotal, `Sortie de matières premières pour production (−${montantTotal} €)`);
    // Écriture 2 — entrée en stock des produits finis (agrégée)
    pushByName(joueur, modifications, "Stocks produits finis", +montantTotal, `Entrée en stock : ${quantiteEffective} produit${quantiteEffective > 1 ? "s" : ""} fini${quantiteEffective > 1 ? "s" : ""} valorisé${quantiteEffective > 1 ? "s" : ""} au coût de production (+${montantTotal} €)`);
    push("productionStockee", montantTotal, `Production stockée : la valeur produite est enregistrée en contrepartie de l'entrée en stock (+${montantTotal} €)`);
    return { succes: true, modifications };
}
/**
 ...
 */
function appliquerRealisationAzura(etat, joueurIdx) {
    const joueur = etat.joueurs[joueurIdx];
    const modifications = [];
    const modele = getModeleValeurEntreprise(joueur.entreprise);
    if (modele.mode !== "negoce") {
    ...
    push("servicesExterieurs", montant, `Coût de canal Azura : commissions marketplace, ads, abonnements outils (+${montant} €)`);
    push("dettes", montant, `Dette fournisseur +${montant} € (règlement au trimestre suivant)`);
    return { succes: true, modifications };
}
/**
 ...
 */
function appliquerRealisationVeloce(etat, joueurIdx) {
    const joueur = etat.joueurs[joueurIdx];
    const modifications = [];
    const modele = getModeleValeurEntreprise(joueur.entreprise);
    if (modele.mode !== "logistique") {
    ...
    const montant = types_1.COUT_APPROCHE_VELOCE_PAR_TOUR;
    pushByName(joueur, modifications, "Stocks en-cours de production", +montant, `Tournée en cours d'exécution : en-cours de production constaté (+${montant} €)`);
    const push = makePush(joueur, modifications);
    push("productionStockee", montant, `Production stockée : valeur de la tournée en cours (+${montant} €)`);
    return { succes: true, modifications };
}
/**
 ...
 */
function appliquerRealisationSynergia(etat, joueurIdx) {
    const joueur = etat.joueurs[joueurIdx];
    const modifications = [];
    const modele = getModeleValeurEntreprise(joueur.entreprise);
    ...
    const montant = types_1.COUT_STAFFING_SYNERGIA_PAR_TOUR;
    pushByName(joueur, modifications, "Stocks en-cours de production", +montant, `Mission en cours de réalisation : en-cours de production constaté (+${montant} €)`);
    const push = makePush(joueur, modifications);
    push("productionStockee", montant, `Production stockée : valeur de la mission en cours (+${montant} €)`);
    return { succes: true, modifications };
}
/**
 ...
 */
function appliquerExtourneEnCours(etat, joueurIdx) {
    const joueur = etat.joueurs[joueurIdx];
    const modifications = [];
    const modele = getModeleValeurEntreprise(joueur.entreprise);
    ...
    pushByName(joueur, modifications, "Stocks en-cours de production", -montant, `Extourne de l'en-cours : la prestation sort du stock en-cours au moment de la facturation (−${montant} €)`);
    const push = makePush(joueur, modifications);
    push("productionStockee", -montant, `Extourne de la production stockée : l'en-cours redevient disponible pour la facturation (−${montant} €)`);
    return { succes: true, modifications };
}
...
function investirCartePersonnelle(etat, joueurIdx, carteId) {
    ...
    for (const effet of carte.effetsImmédiats) {
        ...
        const { ancienneValeur, nouvelleValeur, ligneNom } = appliquerDeltaPoste(joueur, effet.poste, effet.delta);
        modifications.push({
            joueurId: joueur.id,
            poste: effet.poste,
            ancienneValeur,
...
function genererClientsParCommerciaux(joueur) {
    ...
    for (const carte of joueur.cartesActives) {
        if (!carte.clientParTour)
            continue;
        const idCarte = cartes_1.CLIENT_CARD_ID_BY_TYPE[carte.clientParTour];
        if (!idCarte)
            continue;
        const clientCarte = cartes_1.CARTES_CLIENTS.find((c) => c.id === idCarte);
        if (clientCarte) {
