...
function appliquerRealisationBelvaux(etat, joueurIdx) {
    ...
    const montant = quantite * types_1.PRIX_UNITAIRE_MARCHANDISE; // 1 000 €
    ...
    const push = makePush(joueur, modifications);
    push("achats", montant, `Variation de stock matières premières : consommation de ${quantite} unité (+${montant} € en charges)`);
    pushByName(joueur, modifications, "Stocks matières premières", -montant, `Sortie de matières premières pour production (−${montant} €)`);
    // Écriture 2 — entrée en stock des produits finis
    pushByName(joueur, modifications, "Stocks produits finis", +montant, `Entrée en stock : ${quantite} produit fini valorisé au coût de production (+${montant} €)`);
    push("productionStockee", montant, `Production stockée : la valeur produite est enregistrée en contrepartie de l'entrée en stock (+${montant} €)`);
    return { succes: true, modifications };
}
/**
