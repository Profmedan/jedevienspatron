...
### L-B9A-3 — Les erreurs TS2786 / TS2322 lucide-react sont pré-existantes dans le VM

**Contexte** : `npx tsc --noEmit` sur `apps/web` renvoie 165 erreurs TS2786 (Link, Icon, Lucide components "cannot be used as JSX component") dues à un duplicate `@types/react` dans le `node_modules` imbriqué du VM (React 18 project vs React 19 types leak).

**Règle** : lors d'un garde-fou tsc sur `apps/web`, filtrer les erreurs via :
```
