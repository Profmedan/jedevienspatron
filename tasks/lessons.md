...
### L-B8-A — Le plancher 0 sur les produits piège les postes à variation nette (productionStockee)
**Cas concret** : en mode production (Belvaux, Option A), une vente doit pousser `stocks −1 000 / productionStockee −1 000`. Mais `appliquerDeltaPoste` plafonnait **tous** les produits à `Math.max(0, …)`. Résultat : `productionStockee` restait bloqué à 0 et l'équilibre Actif = Passif sautait de −1 000 après chaque vente. Les tests `cycle-shape.test.ts` le détectaient immédiatement.
**Règle** :
- En PCG, `productionStockee` représente la VARIATION nette des stocks de produits finis sur la période. Elle est naturellement signée : positive quand on stocke de la production, négative quand on déstocke un produit fini. Elle **doit** pouvoir être négative.
- Les autres produits (`ventes`, `produitsFinanciers`, `revenusExceptionnels`) gardent leur plancher 0 — ils ne peuvent qu'augmenter ou rester à 0.
- Quand un poste comptable est naturellement à deux signes (capitaux propres négatifs en perte massive → L-B7-C ; trésorerie négative → découvert ; productionStockee déstockage), vérifier systématiquement que le moteur ne le plafonne pas à 0.
- **Test de reproduction** : le plancher 0 est masqué tant que le test part d'une valeur positive suffisante. Il faut un test qui démarre de 0 et fait `delta < 0` pour le débusquer.

### L-B8-A.2 — T25 est un invariant stabilisé : ne pas y toucher en dehors d'un lot T25 explicite
**Cas concret** : pour donner un bilan « cohérent service » (0 stocks) à Véloce / Synergia, j'avais déplacé 4 000 € de Stocks vers Trésorerie → Tréso 14 000 €. Le test `tache25.test.ts` « Trésorerie initiale = 10 000 € pour les 4 entreprises » est immédiatement tombé. Pierre avait pourtant écrit : « on garde de main la structure 8 étapes + fin d'exercice + fixes B7 » et « sans casser ce que main a déjà stabilisé ».
**Règle** :
- Avant de modifier un poste du bilan initial des 4 entreprises, vérifier si un test T25 l'assert-it directement. Si oui, **ne pas** le modifier dans un autre lot que T25.
- En mode service, les stocks initiaux peuvent rester à 4 000 € (buffer de consommables — interprétation métier : papeterie, fournitures IT, pièces détachées, carburant). Ce buffer ne descend jamais car le moteur service ne le consomme pas (`appliquerCarteClient` mode service passe par `servicesExterieurs + / dettes +`, pas par les stocks).
- Principe plus large : **distinguer ce qui est une régression de test** (bug introduit par le changement) **de ce qui est un changement volontaire de spec** (qui doit passer par son propre lot avec mise à jour explicite des tests). En cas de doute, préserver la spec en place.

### L-B8-D — Un test exclu de la compilation pendant des mois = une spec qui dérive en silence
**Cas concret** : `packages/game-engine/tests/engine.test.ts` a cessé de compiler depuis le commit `d8571b4` (retrait de `creerJoueur` et `calculerCoutCommerciaux` de l'API publique). Jest continue pourtant à afficher « 148 / 148 tests passent » parce que le fichier ne compile pas → Jest ne l'exécute pas → Jest le compte comme « non existant ». Entre `d8571b4` et B8-D, plusieurs refactors (auto-recrutement du Commercial Junior par `creerJoueur`, `nbClientsParTour: 2`) ont rendu trois tests statiquement obsolètes. Au moment de réactiver le fichier en B8-D, 5 tests échouent — 3 pour la semantique négoce/production (B8-B) et 2 pour des assertions stales jamais vérifiées depuis des mois.
**Règle** :
- `tests/**` doit compiler à chaque commit. Un test qui ne compile pas n'est pas « temporairement cassé », il est **absent** — et les specs qu'il couvrait deviennent libres de dériver.
- Tout export retiré de l'API publique (`src/index.ts`) doit être re-vérifié contre `tests/**` en même temps : soit le test est rebranché sur une autre fonction, soit le symbole reste exporté localement (`export function` hors `src/index.ts`) pour les imports `../src/engine` des tests.
- Un `npm test` qui affiche un nombre de tests passants en hausse (148 → 199) après activation d'un fichier de tests est un signal positif, mais l'inverse est un piège silencieux : **scruter les "Test Suites: N passed, M total" en plus du décompte de tests**. Si une suite disparaît sans raison documentée, c'est un test désactivé par dérive de typage.
- Appliqué en B8-D : `creerJoueur` et `calculerCoutCommerciaux` ré-exportés avec commentaire explicite « Exporté (B8-D) pour les tests unitaires ». Leur présence hors de `src/index.ts` est volontaire — l'API publique reste minimale, mais la compilation des tests est non-négociable.

### L-B8-D.2 — Tester par mode dans un moteur multi-branches, avec un test de régression du bug initial
**Cas concret** : B8-B a introduit un switch à 3 branches dans `appliquerCarteClient` (négoce / production / service). Sans tests par mode, rien n'empêche une régression silencieuse où l'une des branches ré-active par copier-coller la comptabilité d'une autre. Le bug initial (service → 100 % de marge brute) serait invisible à relire le diff.
**Règle** :
- Dès qu'une fonction a un switch par type métier (mode, secteur, stratégie), créer **un test par branche** dont les assertions vérifient *uniquement* les écritures spécifiques à cette branche ET *excluent* explicitement les écritures des autres branches (assertion « l'achats du négoce n'est pas touché en mode production »). Sans cette exclusion, deux branches peuvent muter le même poste à tort sans faire échouer le test.
- Ajouter un **test de régression** pour le bug spécifique qui a motivé le refactor. Dans B8-B, c'est « la marge brute service doit être < 100 % après un appliquerCarteClient ». Ce test deviendra rouge si quelqu'un recopie la branche négoce dans la branche service par inadvertance.
- Pierre a validé explicitement cet ordre en B8 : **données → UI → tests**. Écrire les tests après la stabilisation des données + UI évite le piège de devoir les réécrire trois fois pendant les itérations de modélisation.

### L-B8-F — `ignoreBuildErrors: true` est un masque ; toujours passer `tsc --noEmit` sur `apps/web` à la fin d'une refonte de types
**Cas concret** : B8-A a rendu `secteurActivite` obligatoire sur `EntrepriseTemplate`. Deux endroits de `apps/web` construisaient des templates sans ce champ (`components/jeu/EntrepriseBuilder.tsx:171` et `app/jeu/hooks/useGamePersistence.ts:354` pour les templates custom chargés de Supabase). Les deux erreurs TS2741 / TS2322 étaient restées **invisibles** pendant B8-A → B8-E parce que `next.config.ts` a `ignoreBuildErrors: true` et que je ne faisais passer `tsc --noEmit` que sur `packages/game-engine`. Détecté uniquement en B8-F, au moment de la vérif finale.
**Règle** :
- Toute refonte qui touche **un type partagé** (`EntrepriseTemplate`, `Joueur`, `EtatJeu`, etc.) doit se conclure par un `cd apps/web && npx tsc --noEmit` obligatoire, même si le build Next passe. `ignoreBuildErrors: true` existe pour la dette lucide-react / recharts, **pas pour nos propres régressions sémantiques**.
...
## Session 2026-04-24 — B9-A (cycle 8 étapes, insertion REALISATION_METIER)

### L-B9A-1 — Garde-fou structurel obligatoire entre refonte de structure et refonte de sémantique

**Contexte** : B9-A = migration structurelle pure (renumérotation des étapes, placeholder no-op). B9-C/D/E = injection de la polymorphie métier réelle. Pierre a imposé un garde-fou entre les deux : `tsc` + smoke test du cycle /8 AVANT de commencer la sémantique.

**Valeur** : le garde-fou a permis de confirmer que le cycle 0→1→2→3→4→5→6→7 traverse sans erreur avec `avancerEtape` et que le placeholder `appliquerRealisationMetier` renvoie bien `{ succes:true, modifications:[] }` — donc aucun effet latéral. Sans cette étape, un bug de structure (mauvais index de fin de tour, mauvais skip-auto, placeholder qui écrit par erreur) aurait été diagnostiqué comme un bug de B9-D et aurait pollué toute la suite.

**Règle** : dans toute refonte qui touche à la fois structure ET sémantique, commiter la structure seule d'abord, vérifier qu'elle tient (tsc + smoke test scripté + tests de régression), PUIS ajouter la sémantique par-dessus. Jamais mélanger les deux dans un seul commit sur un chantier d'ampleur B-série.

### L-B9A-2 — Le smoke test scripté doit appeler le dist compilé, pas le src TS

**Erreur initiale** : premier smoke test écrit avec `import { ... } from "./packages/game-engine"` en chemin relatif depuis `/tmp`, résolution échouée. Deuxième tentative avec signature `initialiserJeu("Belvaux", 8)` obsolète (le moteur prend un tableau `[{pseudo, nomEntreprise}]`). Troisième tentative avec `"Belvaux"` au lieu de `"Manufacture Belvaux"`. Quatrième avec `avancerEtape` supposé retourner un ResultatAction alors qu'il mute en place et retourne `void`.

**Règle** : avant d'écrire un smoke test, inspecter :
1. Le chemin absolu du `dist/index.js`
2. Les exports réels : `node -e "const e=require('<path>'); console.log(Object.keys(e))"`
3. Les signatures exactes dans `engine.ts` (pas les mémoriser de tête)
4. Les noms complets des entreprises dans `entreprises.ts`

Un smoke test qui échoue pour raison de signature n'est pas un garde-fou — c'est un faux garde-fou qui peut masquer un vrai bug si on le corrige à l'arrache.

### L-B9A-3 — ~~Les erreurs TS2786 / TS2322 lucide-react sont pré-existantes dans le VM~~ **OBSOLÈTE depuis commit `05f347e` (B9-E post)**

**Statut** : leçon neutralisée. Le diagnostic initial (erreurs inévitables du VM) était incomplet : la cause racine était un duplicate `@types/react` tiré par `recharts@3.8.1 → react-redux@9.2.0 → @types/react@19.2.14`, hoisté à la racine du monorepo en parallèle du `@types/react@18.3.28` d'apps/web. La résolution TypeScript piochait tantôt dans la v18, tantôt dans la v19 (dont l'union `ReactNode` inclut `bigint`).

**Fix appliqué** : ajout d'un `overrides` sur `@types/react` et `@types/react-dom` à `^18.3.1` dans `package.json` racine. Les `@types` sont des types TS sans runtime — l'override n'entre PAS en conflit avec la règle CLAUDE.md sur `react`/`react-dom` (runtime, 2ᵉ instance). Suppression complète de `node_modules` + `package-lock.json` puis `npm install` pour forcer une résolution propre.

**Résultat** : `tsc apps/web` passe maintenant à EXIT=0 avec 0 erreur. Le garde-fou `tsc apps/web` devient un vrai filet de sécurité pour les prochaines refontes — toute erreur TS remontée à l'avenir sera une vraie erreur, pas du bruit environnemental.

**Leçon transformée** : L-B9E-1 (voir ci-dessous).

### L-B9E-1 — Un "pré-existant" qui masque le vrai filet de sécurité doit être corrigé, pas contourné

**Contexte** : pendant toute la session B9-A → B9-E, le garde-fou `tsc apps/web` remontait 165 erreurs documentées comme "pré-existantes, inévitables dans le VM". J'ai utilisé un filtrage grep pour les écarter à chaque commit. C'était une fausse bonne pratique : une accumulation de 165 erreurs "pré-existantes" finit par noyer les vraies nouvelles erreurs et rend le garde-fou inutile.

**Erreur de raisonnement** : j'ai présumé la cause ("duplicate types dans le VM, pas résoluble") sans exécuter le diagnostic concret (`npm ls @types/react`). Il aurait suffi d'une seule commande pour identifier que c'est `recharts → react-redux` qui tire la v19, et qu'un simple `overrides` règle le problème en une ligne.

**Règle** :
1. Une erreur "pré-existante" doit TOUJOURS être tracée à sa cause racine avant d'être acceptée. L-B9E = `npm ls <paquet>` systématique dès qu'un type TS conflit apparaît sur un lib externe.
2. Si un garde-fou accumule plus de 10 erreurs "pré-existantes", c'est un signal rouge : le garde-fou ne fonctionne plus. Réparer AVANT de continuer à commiter par-dessus.
3. Pour les monorepos npm workspaces : les `overrides` à la racine sont le bon canal pour forcer une résolution TS cohérente sur les `@types/*` (safe car types only, pas de runtime).

**Commande diagnostic** : `npm ls @types/react` ou `npm ls <pkg>` pour voir l'arbre et identifier qui tire quelle version.

**Commande fix type-duplicate** : `overrides` dans le `package.json` racine, puis `rm -rf node_modules package-lock.json && npm install`.
