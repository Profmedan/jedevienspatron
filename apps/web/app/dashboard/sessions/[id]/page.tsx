    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">{dateCreation}</p>
            </div>
            <div className="flex gap-6 text-center">
              <div>
                <div className="text-2xl font-black text-gray-900">{typedSession.nb_tours}</div>
                <div className="text-xs text-gray-500">trimestres</div>
              </div>
              <div>
                <div className="text-2xl font-black text-indigo-600">{nbPlayers}</div>
                <div className="text-xs text-gray-500">joueur{nbPlayers > 1 ? "s" : ""}</div>
              </div>
              <div>
                <div className="text-2xl font-black text-red-500">{nbBankrupts}</div>
                <div className="text-xs text-gray-500">faillite{nbBankrupts > 1 ? "s" : ""}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        {typedSession.status !== "finished" && (
          <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-6 flex items-center justify-between gap-6">
            <div>
              <p className="font-semibold text-indigo-800 mb-1">Session en cours — partagez le code :</p>
              <p className="text-indigo-500 text-sm">
                Les apprenants entrent ce code sur la page d&apos;accueil pour rejoindre.
              </p>
            </div>
            <div className="text-center">
              <div className="font-mono font-black text-4xl text-indigo-700 tracking-[0.2em] bg-white border-2 border-indigo-300 rounded-xl px-6 py-3">
                {typedSession.room_code}
              </div>
            </div>
          </div>
        ...
        {nbPlayers > 0 && (
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
              <div className="text-3xl font-black text-gray-900">{avgScore.toLocaleString("fr-FR")} €</div>
              <div className="text-xs text-gray-500 mt-1">Score moyen</div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
              <div className="text-3xl font-black text-green-600">{maxScore.toLocaleString("fr-FR")} €</div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
              ...
              <div className="text-xs text-gray-500 mt-1">Taux de faillite</div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-gray-900">Classement des apprenants</h2>
            {nbPlayers === 0 && (
              <span className="text-xs text-gray-400">En attente des joueurs…</span>
            )}
          </div>
          ...
          {nbPlayers === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-gray-500 text-sm">Aucun apprenant n&apos;a encore rejoint cette session.</p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          {player.is_bankrupt ? (
                            <span className="inline-flex items-center gap-1 bg-red-100 text-red-600 px-2.5 py-1 rounded-full text-xs font-semibold">
                              💥 Faillite
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 bg-green-100 text-green-600 px-2.5 py-1 rounded-full text-xs font-semibold">
                              ✅ Actif
                            </span>
                          )}
                        </td>
