...
Simple version: Use simple hardcoded BEFORE values for turn 1.
"""
...
def get_before_values(enterprise_name, row_type):
    """Return BEFORE values for an enterprise. Format: actif for immo/stocks/creances/treso, passif for capitaux/emprunts/dettes/decouvert/resultat"""
    values = {
        'Manufacture Belvaux': {
            'actif': [8000, 4000, 2000, 2000],
            'passif': [8000, 8000, 0, 0, 0],
        },
        'Véloce Transports': {
            'actif': [10000, 2000, 1000, 3000],
            'passif': [8000, 8000, 0, 0, 0],
        },
        'Azura Commerce': {
            'actif': [4000, 8000, 2000, 2000],
            'passif': [8000, 8000, 0, 0, 0],
        },
        'Synergia Lab': {
            'actif': [6000, 2000, 3000, 2000],
            'passif': [6500, 6500, 0, 0, 0],
        },
    }
    return values.get(enterprise_name, {'actif': [0, 0, 0, 0], 'passif': [0, 0, 0, 0, 0]})[row_type]
...
def build_turn_block(ws, turn_num):
    ...
    actifs = [
        (40, 'Immobilisations', 0),
        (41, 'Stocks', 1),
        (42, 'Créances clients', 2),
        (43, 'Trésorerie', 3),
        (44, 'TOTAL ACTIF', None),
    ]

    for offset, label, idx in actifs:
        r = bs + offset
        ws[f'A{r}'] = label
        ws[f'A{r}'].alignment = Alignment(horizontal='left')
        ...
        if turn_num == 1:
            if idx is not None:
                # Use INDEX/MATCH to get BEFORE values from lookup table
                actif_values = get_before_values('Manufacture Belvaux', 'actif')
                ws[f'C{r}'] = actif_values[idx]
            else:
    ...
    passifs = [
        (47, 'Capitaux propres', 0),
        (48, 'Emprunts', 1),
        (49, 'Dettes fournisseurs', 2),
        (50, 'Découvert', 3),
        (51, 'Résultat', 4),
        (52, 'TOTAL PASSIF', None),
    ]

    for offset, label, idx in passifs:
        r = bs + offset
        ws[f'A{r}'] = label
        ws[f'A{r}'].alignment = Alignment(horizontal='left')
        ...
        if turn_num == 1:
            if idx is not None:
                passif_values = get_before_values('Manufacture Belvaux', 'passif')
                ws[f'C{r}'] = passif_values[idx]
            else:
